...
function codeLayout(nodes, edges){
...
export default function App(){
  ...
  var _minConf=useState(0),minConf=_minConf[0],setMinConf=_minConf[1];
  var _hidePre=useState(false),hidePrereq=_hidePre[0],setHidePrereq=_hidePre[1];
  var _focusOn=useState(false),focusOn=_focusOn[0],setFocusOn=_focusOn[1];
  var _allLinks=useState(false),showAllLinks=_allLinks[0],setShowAllLinks=_allLinks[1];
  var _qi=useState(0),quizI=_qi[0],setQuizI=_qi[1];
  var _qf=useState(false),quizFlip=_qf[0],setQuizFlip=_qf[1];
  ...
  var vn=useMemo(function(){return nodes.filter(function(n){return visIds.has(n.id)&&(n.confidence==null||n.confidence>=minConf)&&(!hidePrereq||n.in_text!==false);});},[nodes,visIds,minConf,hidePrereq]);
  var focusSet=useMemo(function(){if(!focusOn||!selId)return null;var s=new Set([selId]);edges.forEach(function(e){if(e.source===selId)s.add(e.target);if(e.target===selId)s.add(e.source);});return s;},[focusOn,selId,edges]);
  ...
  /* ── Palace: quizcards generated from the map's concepts ── */
  var quizDeck=useMemo(function(){return nodes.filter(function(n){return n.description&&n.description.length>4;});},[nodes]);
  var palaceView=view==='palace'?h("div",{key:"pl",style:{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,padding:24}},
    h("div",{style:{display:"flex",alignItems:"center",gap:10}},h("div",{style:{fontSize:30}},"🏛️"),h("h2",{style:{fontSize:20,fontWeight:600}},"Memory Palace — Quizcards")),
    quizDeck.length?(function(){
      var card=quizDeck[Math.min(quizI,quizDeck.length-1)];var t=tcolor(card.concept_type);
      return h("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",gap:14,width:"100%",maxWidth:460}},
        h("div",{style:{fontSize:12,color:DIM}},(Math.min(quizI,quizDeck.length-1)+1)+" / "+quizDeck.length),
        h("div",{onClick:function(){setQuizFlip(!quizFlip);},style:{width:"100%",minHeight:200,background:SURF,border:"1.5px solid "+t.a,borderRadius:16,padding:"26px 24px",cursor:"pointer",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",textAlign:"center",gap:12,boxShadow:"0 6px 24px rgba(0,0,0,0.12)"}},
          h("span",{style:{fontSize:10,fontWeight:600,letterSpacing:0.5,textTransform:"uppercase",color:t.a}},quizFlip?"Definition":(card.concept_type||"concept")),
          quizFlip?h("div",{style:{fontSize:15,lineHeight:1.55,color:TXT}},card.description):h("div",{style:{fontSize:22,fontWeight:700,color:TXT}},card.label),
          h("div",{style:{fontSize:11,color:MUT}},quizFlip?"(tap to hide)":"What does this mean? — tap to reveal")),
        h("div",{style:{display:"flex",gap:8}},
          h("button",{onClick:function(){setQuizFlip(false);setQuizI((quizI-1+quizDeck.length)%quizDeck.length);},style:Object.assign({padding:"7px 16px"},B(DIM,"transparent"))},"‹ Prev"),
          h("button",{onClick:function(){setQuizFlip(!quizFlip);},style:Object.assign({padding:"7px 16px"},B("#A29BFE","rgba(162,155,254,0.12)"))},quizFlip?"Hide":"Reveal"),
          h("button",{onClick:function(){setQuizFlip(false);setQuizI((quizI+1)%quizDeck.length);},style:Object.assign({padding:"7px 16px"},B(DIM,"transparent"))},"Next ›"),
          h("button",{onClick:function(){setQuizFlip(false);setQuizI(Math.floor(Math.random()*quizDeck.length));},style:Object.assign({padding:"7px 16px"},B("#00B8A9","rgba(0,184,169,0.12)"))},"Shuffle")),
        h("div",{style:{fontSize:11,color:MUT,textAlign:"center",maxWidth:380,lineHeight:1.5}},"Cards are generated from the concepts in your current map. Open a different map in Library to study its concepts."));
    })():h("p",{style:{fontSize:14,color:DIM,maxWidth:380,textAlign:"center",lineHeight:1.6}},"No concepts loaded yet. Open a map from the Library (or upload one) and its concepts become quizcards here.")):null;
  ...
  var graphGuts=view==='graph'?[
    ...
    h("svg",{key:"svg",ref:svgRef,style:{position:"absolute",inset:0,width:"100%",height:"100%",zIndex:1,overflow:"visible"}},
      ...
      dispNodes.map(function(n){var t=tcolor(n.concept_type);var isSel=selId===n.id,isHov=hovId===n.id;var masked=studyMode==='review'&&revealed&&!revealed.has(n.id);var soil=studyMode==='soil';var llines=masked?["• • •"]:(n.ll||[]);var dl=((soil||showD)&&!masked)?(n.dl||[]):[];if(soil){if(soilDef==='off'&&!isSel)dl=[];else if(soilDef==='brief'&&!isSel)dl=dl.slice(0,3);}var totalH=(n.lh||30)+(dl.length?dl.length*16+20:0);var sx2=n.x*cam.z+cam.x,sy2=n.y*cam.z+cam.y;var lSz=Math.round(impSize(n.id,14)*fontScale),dSz=Math.round(impSize(n.id,10)*fontScale);var shp=shapeForType(n.concept_type);var sbox=shapeBox(shp,n.w,totalH);var imp=0.85+(deg[n.id]||0)/Math.max(maxDeg,1)*0.45;
        return h("g",{key:n.id,transform:"translate("+sx2+","+sy2+") scale("+(cam.z*imp)+")",style:{cursor:tool==='select'||tool==='magnify'?'pointer':'inherit',opacity:(focusSet&&!focusSet.has(n.id))?0.16:1,animation:studyMode==='grow'?('growFade '+(Math.max(150,growSpeed)/1000)+'s ease both'):undefined},
          ...
          (!masked&&ch[n.id]&&ch[n.id].length)?h("g",{transform:"translate(0,"+(totalH/2+12)+")",style:{cursor:"pointer"},onPointerDown:function(ev){ev.stopPropagation();},onClick:function(ev){ev.stopPropagation();setColl(function(prev){var n2=new Set(prev);if(n2.has(n.id))n2.delete(n.id);else n2.add(n.id);return n2;});}},h("circle",{r:9,fill:SURF,stroke:t.b,strokeWidth:1.5}),h("path",{d:collapsed.has(n.id)?"M-4 0H4M0 -4V4":"M-4 0H4",stroke:t.b,strokeWidth:1.8,strokeLinecap:"round"}),collapsed.has(n.id)?h("text",{x:13,y:4,fontSize:10/1,fill:DIM,style:{userSelect:"none"}},(ch[n.id]||[]).length):null):null,
    ...
    selN?(function(){var sc=w2s(selN.x,selN.y);var t=tcolor(selN.concept_type);var rc=cRef.current?cRef.current.getBoundingClientRect():{width:800};var cW=260;var cx2=Math.min(Math.max(8,sc.x+70),rc.width-cW-12);var cy2=Math.max(8,sc.y-40);
      return h("div",{key:"dc", onPointerDown:function(e){e.stopPropagation();}, style:{position:"absolute",left:cx2,top:cy2,width:cW,background:SURF,border:"1px solid "+BRD,borderRadius:12,padding:"12px 14px",boxShadow:"0 6px 24px rgba(0,0,0,0.25)",zIndex:20,maxHeight:"50vh",overflowY:"auto"}},
        ...
        (selN.canonical_id)?h("div",{style:{display:"flex",alignItems:"center",gap:5,marginBottom:6,padding:"3px 8px",background:"rgba(0,184,169,0.10)",border:"1px solid rgba(0,184,169,0.35)",borderRadius:6,fontSize:10.5,color:"#0A9E90"}},h("span",{style:{fontWeight:700}},"grounded"),h("span",{style:{opacity:0.85,fontFamily:"monospace"}},selN.canonical_id),h("span",{style:{marginLeft:"auto",opacity:0.7}},Math.round((selN.link_score||0)*100)+"%")):null,
    ...
    showSettings?h("div",{key:"st",style:{position:"absolute",top:8,right:8,width:240,background:SURF,border:"1px solid "+BRD,borderRadius:12,padding:"14px 16px",boxShadow:"0 6px 24px rgba(0,0,0,0.2)",zIndex:25,maxHeight:"70vh",overflowY:"auto"}},
      ...
      h("div",{style:{fontSize:12,fontWeight:600,color:DIM,marginBottom:6}},"VIEW & FILTER"),
      h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}},h("span",{style:{fontSize:11,color:MUT}},"Focus selected concept"),h("button",{onClick:function(){setFocusOn(!focusOn);},style:{padding:"2px 10px",borderRadius:10,fontSize:10,cursor:"pointer",border:"1px solid "+(focusOn?"rgba(162,155,254,0.4)":BRD),background:focusOn?"rgba(162,155,254,0.15)":"transparent",color:focusOn?"#A29BFE":DIM}},focusOn?"On":"Off")),
      h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}},h("span",{style:{fontSize:11,color:MUT}},"Show all links (else fade unselected)"),h("button",{onClick:function(){setShowAllLinks(!showAllLinks);},style:{padding:"2px 10px",borderRadius:10,fontSize:10,cursor:"pointer",border:"1px solid "+(showAllLinks?"rgba(162,155,254,0.4)":BRD),background:showAllLinks?"rgba(162,155,254,0.15)":"transparent",color:showAllLinks?"#A29BFE":DIM}},showAllLinks?"On":"Off")),
      h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}},h("span",{style:{fontSize:11,color:MUT}},"Hide prerequisites (not in text)"),h("button",{onClick:function(){setHidePrereq(!hidePrereq);},style:{padding:"2px 10px",borderRadius:10,fontSize:10,cursor:"pointer",border:"1px solid "+(hidePrereq?"rgba(253,203,110,0.4)":BRD),background:hidePrereq?"rgba(253,203,110,0.15)":"transparent",color:hidePrereq?"#FDCB6E":DIM}},hidePrereq?"On":"Off")),
      h("div",{style:{fontSize:11,color:MUT,marginBottom:3}},"Min confidence  "+Math.round(minConf*100)+"%"),
      h("input",{type:"range",min:"0",max:"0.9",step:"0.05",value:minConf,onChange:function(e){setMinConf(parseFloat(e.target.value));},style:{width:"100%",marginBottom:12,accentColor:"#A29BFE"}}),
