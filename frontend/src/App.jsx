...
export default function App(){
  ...
  useEffect(function(){
    function fn(e){
      ...
      if(e.key==='Escape'){setSel(null);setTool('select');setEf(null);}
  ...
  var cursor=tool==='draw'?'crosshair':tool==='eraser'?'cell':tool==='magnify'?'zoom-in':tool==='hand'?(dragState?'grabbing':'grab'):'default';
  ...
  },[dragState,cam,vn,s2w,drawPath,tool,setData]);
  ...
  var headerView=h("header",{key:"hd",style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",background:SURF,borderBottom:"1px solid "+BRD,flexShrink:0,height:44}},
    ...
    view==='graph'?h("div",{style:{display:"flex",gap:2,alignItems:"center"}},
      h("span",{style:{fontSize:11,color:DIM,marginRight:6}},vn.length+"·"+ve.length),
      [{k:'select',l:'↖',t:'Select (V)'},{k:'hand',l:'✋',t:'Pan (H)'},{k:'magnify',l:'🔍',t:'Zoom (M)'},{k:'draw',l:'✎',t:'Draw (D)'},{k:'eraser',l:'⌫',t:'Erase (E)'}].map(function(b){return h("button",{key:b.k,title:b.t,onClick:function(){setTool(b.k);},style:{padding:"3px 7px",borderRadius:5,border:tool===b.k?"1px solid "+TXT+"30":"1px solid transparent",background:tool===b.k?BG:"transparent",color:tool===b.k?TXT:DIM,fontSize:14,cursor:"pointer"}},b.l);}),
  ...
  var helpView=view==='help'?h("div",{key:"hp",style:{flex:1,padding:24,overflowY:"auto",maxWidth:560,margin:"0 auto"}},
    ...
    [["How do I create a mindmap?","Upload a PDF on the Home tab. AI extracts concepts automatically."],["How do I edit?","Click a concept, then click the label or description to type."],["Tools?","↖ Select, ✋ Hand (pan only), 🔍 Magnify, ✎ Draw, ⌫ Erase."],["Credits?","Upload +5, Confirm +10, Share +15, Edit +1, Upvote +3."],["Shortcuts?","V=select H=hand M=magnify D=draw E=erase Ctrl+Z=undo Del=remove"]].map(function(qa,i){return h("div",{key:i,style:{marginBottom:8,padding:12,background:SURF,border:"1px solid "+BRD,borderRadius:8}},h("div",{style:{fontSize:14,fontWeight:600,marginBottom:2}},qa[0]),h("div",{style:{fontSize:13,color:MUT,lineHeight:1.5}},qa[1]));}),
  ...
  var graphGuts=view==='graph'?[
    ...
    selN?(function(){var sc=w2s(selN.x,selN.y);var t=typeColor(P,selN.concept_type);var rc=cRef.current?cRef.current.getBoundingClientRect():{width:800};var cW=260;var cx2=Math.min(Math.max(8,sc.x+70),rc.width-cW-12);var cy2=Math.max(8,sc.y-40);
      return h("div",{key:"dc",style:{position:"absolute",left:cx2,top:cy2,width:cW,background:SURF,border:"1px solid "+BRD,borderRadius:12,padding:"12px 14px",boxShadow:"0 6px 24px rgba(0,0,0,0.25)",zIndex:20,maxHeight:"50vh",overflowY:"auto"}},
        ...
        h("div",{style:{display:"flex",gap:4,marginBottom:6}},h("button",{onClick:function(){submitCorrection({map_id:mapId,type:"approve",original:{id:selId}});},style:B("#51CF66","rgba(81,207,102,0.1)")},"✓"),h("button",{onClick:function(){setData(function(dd){return{nodes:dd.nodes.filter(function(nd){return nd.id!==selId;}),edges:dd.edges.filter(function(ed){return ed.source!==selId&&ed.target!==selId;}),drawings:dd.drawings};});setSel(null);},style:B("#FF6B6B","rgba(255,107,107,0.1)")},"✗")),
        connE.length>0?h("div",null,h("div",{style:{fontSize:11,color:DIM,fontWeight:600,marginBottom:3}},"Connections ("+connE.length+")"),connE.slice(0,8).map(function(e,i){var isSrc=e.source===selId,oId=isSrc?e.target:e.source,o=nm[oId];var cat=edgeCat(e.relation_type),es=P.edges[cat]||P.edges.custom;return h("div",{key:i,onClick:function(ev){ev.stopPropagation();zoomTo(oId);},style:{padding:"4px 6px",background:BG,borderRadius:4,marginBottom:2,borderLeft:"2px solid "+es.color,cursor:"pointer",fontSize:11}},h("span",{style:{color:es.color,fontWeight:600,fontSize:9,textTransform:"uppercase"}},(e.relation_type||'').replace(/_/g,' '))," ",h("span",{style:{color:DIM}},(isSrc?"→":"←")+" "+(o?o.label:"?")));})):null);})():null,
    /* Legend */
