...
var B=function(c,bg){return{padding:"10px 18px",background:bg||"rgba(162,155,254,0.12)",border:"1px solid "+(c?c+"40":"rgba(162,155,254,0.3)"),borderRadius:8,color:c||"#A29BFE",fontSize:14,fontWeight:500,cursor:"pointer",fontFamily:"inherit"}};
...
var hr=useReducer(histR,{past:[],present:init,future:[]});var hist=hr[0],dispatch=hr[1];
...
var S=useState;
var pal=S("aurora"),view=S("home"),sel=S(null),hov=S(null),mapId=S(null),maps=S([]),
upl=S(false),prog=S(null),coll=S(new Set()),ef=S(null),ev=S(''),
cam=S({x:0,y:0,z:0.75}),drag=S(null),tool=S('select'),dp=S(null),dc=S('#A29BFE'),
user=S(null),cmaps=S([]),shareM=S(null),shareDom=S("general"),commDom=S("all"),
authMode=S("login"),authU=S(""),authP=S(""),
onboard=S(!localStorage.getItem("mycel_onboarded")),
fbCat=S("general"),fbText=S(""),fbSent=S(false),
leaders=S([]),comments=S([]),commentText=S(""),commentMap=S(null),
darkMode=S(localStorage.getItem("mycel_theme")!=="light");

var P=PALETTES[pal[0]];
// Theme override for light mode
var BG=darkMode[0]?P.bg:"#F5F0E8";
var SURF=darkMode[0]?P.surface:"#FFFBF5";
var BRD=darkMode[0]?P.border:"#D4CCBB";
var TXT=darkMode[0]?P.text:"#2C2417";
var DIM=darkMode[0]?P.dim:"#9B8E7D";
var MUT=darkMode[0]?P.muted:"#6B5E4D";
...
getLeaderboard().then(function(d){leaders[1](d.users||[])}).catch(function(){});}
},[view[0]]);
...
var ax=1e9,ay=1e9,bx=-1e9,by=-1e9;for(var i=0;i<nl.length;i++){var r=nl[i].r||60;ax=Math.min(ax,nl[i].x-r);ay=Math.min(ay,nl[i].y-r);bx=Math.max(bx,nl[i].x+r);by=Math.max(by,nl[i].y+r)}
...
var handleUpload=function(file){if(!file)return;var ext=file.name.split('.').pop().toLowerCase();
if(['pdf','docx','txt','md','epub'].indexOf(ext)<0)return;
upl[1](true);prog[1]({stage:'uploading',progress:0,message:'Uploading...'});
uploadPDF(file).then(function(r){if(r.nodes){var edgesN=r.edges.map(function(e){return Object.assign({},e,{source:e.source_id||e.source,target:e.target_id||e.target})});
...
var loadMap=function(id){getMap(id).then(function(r){if(r.nodes){var edgesN=r.edges.map(function(e){return Object.assign({},e,{source:e.source_id||e.source,target:e.target_id||e.target})});
var laid=organicLayout(r.nodes,edgesN);dispatch({type:'INIT',data:{nodes:laid,edges:edgesN,drawings:[]}});
...
// Importance-based font size
var impSize=function(nid,base){var d=deg[nid]||0;var imp=d/Math.max(maxDeg,1);return Math.round(base+imp*10)};

// Pointer handlers (same as before, plus magnify on click)
...
else{drag[1]({t:'p',sx:sx,sy:sy,cx:cam[0].x,cy:cam[0].y})}},[vn,s2w,cam[0],nm,edges,tool[0],dc[0],setData]);
...
setData(function(dd){return Object.assign({},dd,{nodes:dd.nodes.map(function(n){if(n.id===drag[0].nid)return Object.assign({},n,{x:nx,y:ny});if(drag[0].off[n.id])return Object.assign({},n,{x:nx+drag[0].off[n.id].dx,y:ny+drag[0].off[n.id].dy});return n})})})}},[drag[0],cam[0],vn,s2w,dp[0],tool[0],setData]);
...
// Click-to-magnify: when selecting a node, zoom to it
var selectAndMagnify=function(nid,e){if(e)e.stopPropagation();
if(sel[0]===nid){sel[1](null);return}sel[1](nid);
var n=nm[nid];if(!n||!cRef.current)return;
var rc=cRef.current.getBoundingClientRect();
cam[1]({x:-n.x*1.5+rc.width/2,y:-n.y*1.5+rc.height/2,z:1.5})};

var selN=sel[0]?nm[sel[0]]:null;
var connE=selN?ve.filter(function(e){return e.source===sel[0]||e.target===sel[0]}):[];
var showD=cam[0].z>0.4,showTm=cam[0].z>0.5,showEL=cam[0].z>0.6;
var stages={uploading:"Uploading",parsing:"Parsing",chunking:"Splitting",extract:"Extracting",validate:"Validating",done:"Complete"};
...
// ONBOARDING OVERLAY
onboard[0]&&h("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200},onClick:function(){onboard[1](false);localStorage.setItem("mycel_onboarded","1")}},
...
"1. Create an account in the ",h("b",null,"Account")," tab",h("br"),
"2. Upload a PDF/DOCX on the ",h("b",null,"Home")," tab",h("br"),
"3. AI extracts concepts → interactive mindmap appears",h("br"),
"4. Click concepts to edit, drag to rearrange",h("br"),
"5. Confirm good maps → share with the community",h("br"),
"6. ",h("b",null,"V"),"=select ",h("b",null,"D"),"=draw ",h("b",null,"E"),"=erase ",h("b",null,"Ctrl+Z"),"=undo"),
h("button",{onClick:function(){onboard[1](false);localStorage.setItem("mycel_onboarded","1")},style:Object.assign({width:"100%",marginTop:8},B())},"Get Started"))),

// HEADER
h("header",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 18px",background:SURF,borderBottom:"1px solid "+BRD,flexShrink:0,gap:10}},
...
h("span",{style:{fontSize:12,color:DIM,marginRight:8}},vn.length+" nodes · "+ve.length+" edges"),
[{k:'select',l:'↖ Select'},{k:'draw',l:'✎ Draw'},{k:'eraser',l:'⌫ Erase'}].map(function(b){return h("button",{key:b.k,onClick:function(){tool[1](b.k)},style:{padding:"6px 12px",borderRadius:6,border:tool[0]===b.k?"1px solid "+TXT+"30":"1px solid transparent",background:tool[0]===b.k?BG:"transparent",color:tool[0]===b.k?TXT:DIM,fontSize:13,cursor:"pointer"}},b.l)}),
tool[0]==='draw'&&["#A29BFE","#5EECD5","#F0A08A","#FDCB6E","#FD79A8","#E8ECF4"].map(function(c){return h("div",{key:c,onClick:function(){dc[1](c)},style:{width:18,height:18,borderRadius:"50%",background:c,cursor:"pointer",outline:dc[0]===c?"2px solid #fff":"none",outlineOffset:2,marginLeft:2}})}),
h("div",{style:{width:1,height:18,background:BRD,margin:"0 6px"}}),
h("button",{onClick:addNode,style:{padding:"6px 12px",borderRadius:6,border:"1px solid "+BRD,background:"transparent",color:TXT,fontSize:13,cursor:"pointer"}},"+Node"),
h("div",{style:{width:1,height:18,background:BRD,margin:"0 6px"}}),
h("button",{onClick:undo,style:{padding:"6px 10px",borderRadius:6,border:"1px solid "+BRD,background:"transparent",color:hist.past.length?TXT:DIM,fontSize:13,cursor:"pointer",opacity:hist.past.length?1:0.4}},"↩ Undo"),
h("button",{onClick:redo,style:{padding:"6px 10px",borderRadius:6,border:"1px solid "+BRD,background:"transparent",color:hist.future.length?TXT:DIM,fontSize:13,cursor:"pointer",opacity:hist.future.length?1:0.4}},"↪ Redo")),
// Right side: theme toggle + user
h("div",{style:{display:"flex",alignItems:"center",gap:8}},
h("button",{onClick:function(){var next=!darkMode[0];darkMode[1](next);localStorage.setItem("mycel_theme",next?"dark":"light")},style:{padding:"6px 10px",borderRadius:6,border:"1px solid "+BRD,background:"transparent",color:DIM,fontSize:13,cursor:"pointer"}},darkMode[0]?"☀ Light":"🌙 Dark"),
user[0]&&h("div",{style:{fontSize:13,color:"#A29BFE",cursor:"pointer"},onClick:function(){view[1]('account')}},user[0].display_name+" · "+user[0].points+"pts"))),

// HOME
view[0]==='home'&&h("div",{style:{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:24,padding:"40px 20px"}},
...
user[0]?h("div",{onClick:function(){if(!upl[0]){var el=document.getElementById('fi');if(el)el.click()}},style:{width:"100%",maxWidth:480,border:"2px dashed "+BRD,borderRadius:16,padding:"32px 24px",textAlign:"center",cursor:upl[0]?"wait":"pointer"}},
...
prog[0]&&prog[0].stage!=='done'?h("div",null,h("div",{style:{fontSize:16,fontWeight:600,marginBottom:6}},stages[prog[0].stage]||'Processing...'),h("div",{style:{fontSize:13,color:DIM,marginBottom:10}},prog[0].message),h("div",{style:{height:6,background:BG,borderRadius:3,overflow:"hidden",maxWidth:280,margin:"0 auto"}},h("div",{style:{height:"100%",width:Math.max((prog[0].progress||0)*100,3)+"%",background:"linear-gradient(90deg,#6C5CE7,#00B8A9)",borderRadius:3}}))):h("div",null,h("div",{style:{fontSize:17,fontWeight:500,marginBottom:6}},"Drop a file or click to upload"),h("div",{style:{fontSize:14,color:DIM}},"PDF, DOCX, TXT, MD, EPUB supported")))
:h("div",{style:{padding:32,textAlign:"center"}},h("p",{style:{fontSize:16,color:MUT,marginBottom:16}},"Create an account to start uploading"),h("button",{onClick:function(){view[1]('account')},style:B()},"Go to Account")),
h("button",{onClick:function(){view[1]('library')},style:B(DIM,"transparent")},"Browse library")),

// LIBRARY
view[0]==='library'&&h("div",{style:{flex:1,padding:28,overflowY:"auto"}},
h("h2",{style:{fontSize:20,fontWeight:600,marginBottom:18}},"Your Library"),
!user[0]?h("div",{style:{textAlign:"center",padding:40,color:DIM}},"Log in to see your private maps.",h("br"),h("br"),h("button",{onClick:function(){view[1]('account')},style:B()},"Go to Account")):
maps[0].length===0?h("div",{style:{textAlign:"center",padding:40,color:DIM}},"No maps yet. Upload a file on the Home tab."):
...
h("span",{style:{fontSize:11,padding:"4px 12px",borderRadius:10,background:m.status==='confirmed'?'rgba(81,207,102,0.15)':'rgba(90,100,120,0.2)',color:m.status==='confirmed'?'#51CF66':DIM,fontWeight:500}},m.status==='confirmed'?'Confirmed':'Draft')),
...
m.status!=='confirmed'?h("button",{onClick:function(){confirmMap(m.id).then(function(){getMaps().then(function(d){maps[1](d.maps||[])})})},style:B("#51CF66","rgba(81,207,102,0.1)")},"✓ Confirm"):
h("button",{onClick:function(){unconfirmMap(m.id).then(function(){getMaps().then(function(d){maps[1](d.maps||[])})})},style:B(DIM,"transparent")},"Unconfirm"),
m.status==='confirmed'&&h("button",{onClick:function(){shareM[1]({id:m.id,title:m.title||m.filename})},style:B("#A29BFE","rgba(162,155,254,0.1)")},"Share"),
...
h("div",{style:{display:"flex",gap:8}},h("button",{onClick:function(){shareMap(shareM[0].id,shareM[0].title,'',shareDom[0]).then(function(){shareM[1](null)})},style:Object.assign({flex:1},B())},"Share"),h("button",{onClick:function(){shareM[1](null)},style:Object.assign({flex:1},B(DIM,"transparent"))},"Cancel"))))),

// COMMUNITY
view[0]==='community'&&h("div",{style:{flex:1,padding:28,overflowY:"auto"}},
...
cmaps[0].length===0?h("div",{style:{textAlign:"center",padding:40,color:DIM}},"No community maps yet. Confirm and share yours!"):
...
h("div",{style:{display:"flex",alignItems:"center",gap:8,marginBottom:6}},h("div",{style:{fontSize:17,fontWeight:600,flex:1}},m.title),h("span",{style:{fontSize:11,padding:"4px 12px",borderRadius:10,background:"rgba(162,155,254,0.15)",color:"#A29BFE"}},m.domain||'general')),
m.description&&h("div",{style:{fontSize:13,color:MUT,marginBottom:6,lineHeight:1.5}},m.description),
h("div",{style:{fontSize:12,color:DIM,marginBottom:12}},"by "+(m.user_id||'anon')+" · "+fmtDate(m.created_at)),
h("div",{style:{display:"flex",gap:6}},h("button",{onClick:function(){upvoteCommunityMap(m.id).then(function(){loadComm(commDom[0])})},style:B("#FDCB6E","rgba(253,203,110,0.1)")},"↑ "+(m.upvotes||0)),h("button",{onClick:function(){loadMap(m.map_id)},style:B("#5EECD5","rgba(94,236,213,0.1)")},"Open")))})),
// Leaderboard
leaders[0].length>0&&h("div",{style:{marginTop:28}},h("h3",{style:{fontSize:17,fontWeight:600,marginBottom:12}},"Top Contributors"),h("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8}},leaders[0].slice(0,10).map(function(u2,i){return h("div",{key:u2.id,style:{padding:12,background:SURF,border:"1px solid "+BRD,borderRadius:10,display:"flex",alignItems:"center",gap:10}},
...
// HELP / Q&A / FEEDBACK
view[0]==='help'&&h("div",{style:{flex:1,padding:28,overflowY:"auto",maxWidth:600,margin:"0 auto"}},
h("h2",{style:{fontSize:20,fontWeight:600,marginBottom:18}},"Help & Feedback"),
// FAQ
h("div",{style:{marginBottom:28}},
h("h3",{style:{fontSize:17,fontWeight:600,marginBottom:12}},"Frequently Asked Questions"),
[["How do I create a mindmap?","Upload a PDF, DOCX, or TXT file on the Home tab. AI will extract key concepts and their relationships automatically."],
["How do I edit concepts?","Click any concept in the graph view to select it. Click the label or description in the detail card to edit."],
["What do the colors mean?","Each color represents a concept type: purple=theory, teal=definition, blue=method, orange=example, pink=phenomenon."],
["How do I earn points?","Upload (+5), Confirm (+10), Share (+15), Edit (+1), Receive upvote (+3). Levels: Beginner→Experienced→Expert→Professional→Organizer."],
["How do I share a map?","First Confirm it in Library, then click Share to post to the Community."],
["Keyboard shortcuts?","V=select, D=draw, E=erase, Ctrl+Z=undo, Ctrl+Y=redo, Delete=remove, Escape=deselect"]
].map(function(qa,i){return h("div",{key:i,style:{marginBottom:12,padding:14,background:SURF,border:"1px solid "+BRD,borderRadius:10}},
h("div",{style:{fontSize:15,fontWeight:600,marginBottom:4}},qa[0]),
h("div",{style:{fontSize:14,color:MUT,lineHeight:1.5}},qa[1]))})),
// Feedback form
h("h3",{style:{fontSize:17,fontWeight:600,marginBottom:12}},"Send Feedback"),
fbSent[0]?h("div",{style:{padding:20,background:"rgba(81,207,102,0.1)",border:"1px solid rgba(81,207,102,0.2)",borderRadius:10,textAlign:"center",color:"#51CF66",fontSize:15}},"Thank you for your feedback!"):
h("div",null,
h("select",{value:fbCat[0],onChange:function(e){fbCat[1](e.target.value)},style:{width:"100%",padding:"10px 14px",background:BG,border:"1px solid "+BRD,borderRadius:8,color:TXT,fontSize:14,marginBottom:8}},
["general","bug","feature","improvement","other"].map(function(c){return h("option",{key:c,value:c},c.charAt(0).toUpperCase()+c.slice(1))})),
h("textarea",{value:fbText[0],onChange:function(e){fbText[1](e.target.value)},placeholder:"Tell us what you think, report a bug, or suggest a feature...",rows:4,style:{width:"100%",padding:"10px 14px",background:BG,border:"1px solid "+BRD,borderRadius:8,color:TXT,fontSize:14,lineHeight:1.5,resize:"vertical",marginBottom:8}}),
h("button",{onClick:function(){if(fbText[0].trim()){postFeedback(fbCat[0],fbText[0]).then(function(){fbSent[1](true);fbText[1]("")})}},style:Object.assign({width:"100%"},B())},"Submit Feedback"))),

// PALACE
view[0]==='palace'&&h("div",{style:{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:20}},
h("div",{style:{fontSize:56}},"🏛️"),
h("h2",{style:{fontSize:24,fontWeight:600}},"Memory Palace"),
h("p",{style:{fontSize:16,color:DIM,maxWidth:420,textAlign:"center",lineHeight:1.7}},"Walk through your knowledge as a 3D architectural space. Each concept becomes a room, each connection a corridor. Explore, get lost, and learn."),
h("div",{style:{fontSize:14,color:DIM,padding:"10px 20px",border:"1px solid "+BRD,borderRadius:10}},"🚧 Under active development — coming soon")),

// ACCOUNT
view[0]==='account'&&h("div",{style:{flex:1,padding:32,maxWidth:500,margin:"0 auto",overflowY:"auto"}},
user[0]?h("div",null,
h("h2",{style:{fontSize:20,fontWeight:600,marginBottom:18}},"Your Account"),
...
h("div",{style:{display:"flex",gap:24,marginBottom:14}},
...
h("span",{style:{fontSize:14,color:MUT}},"Theme"),
h("button",{onClick:function(){var next=!darkMode[0];darkMode[1](next);localStorage.setItem("mycel_theme",next?"dark":"light")},style:B(DIM,"transparent")},darkMode[0]?"☀ Switch to Light":"🌙 Switch to Dark")),
h("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between"}},
h("span",{style:{fontSize:14,color:MUT}},"Show onboarding guide"),
h("button",{onClick:function(){onboard[1](true)},style:B(DIM,"transparent")},"Show again"))),
// Credit guide
h("div",{style:{padding:20,background:SURF,borderRadius:14,border:"1px solid "+BRD,marginBottom:18}},
h("h3",{style:{fontSize:16,fontWeight:600,marginBottom:10}},"How Credits Work"),
h("div",{style:{fontSize:14,color:MUT,lineHeight:1.8}},
"Upload: +5 · Confirm: +10 · Share: +15 · Edit: +1 · Upvote received: +3 · Comment: +2",h("br"),
"All actions reversible (points deducted on undo)",h("br"),h("br"),
"Beginner (0) → Experienced (50) → Expert (200) → Professional (500) → Organizer (1500)")),
...
h("button",{onClick:function(){var fn=authMode[0]==="login"?login:register;fn(authU[0],authP[0]).then(function(d){
if(d.user){localStorage.setItem("mycel_uid",d.user.id);user[1](d.user)}
else if(d.user_id){localStorage.setItem("mycel_uid",d.user_id);getMe().then(function(r){if(r.user)user[1](r.user)})}
else{alert(d.error||"Failed")}})},style:Object.assign({width:"100%",marginBottom:10},B())},authMode[0]==="login"?"Log In":"Create Account"),
h("button",{onClick:function(){authMode[1](authMode[0]==="login"?"register":"login")},style:Object.assign({width:"100%"},B(DIM,"transparent"))},authMode[0]==="login"?"Need an account? Register":"Have an account? Log in"))),
...
isSel&&h("rect",{x:-n.w/2-6,y:-totalH/2-6,width:n.w+12,height:totalH+12,rx:14,fill:SURF,stroke:t.a,strokeWidth:2,opacity:0.95}),
isHov&&!isSel&&h("rect",{x:-n.w/2-4,y:-totalH/2-4,width:n.w+8,height:totalH+8,rx:12,fill:"none",stroke:t.a,strokeWidth:1,opacity:0.35,strokeDasharray:"4 3"}),
h("circle",{cx:-n.w/2+6,cy:-totalH/2+6,r:4,fill:t.a,opacity:isSel?1:0.7}),
(n.ll||[]).map(function(line,li){return h("text",{key:"l"+li,x:0,y:-totalH/2+20+li*24,textAnchor:"middle",dominantBaseline:"central",fontSize:labelSize,fontWeight:"600",fill:t.a,fontFamily:"'Inter',sans-serif",style:{pointerEvents:"none"}},line)}),
dl.map(function(line,di){return h("text",{key:"d"+di,x:0,y:-totalH/2+(n.lh||30)+8+di*16,textAnchor:"middle",dominantBaseline:"central",fontSize:descSize,fill:t.s,opacity:0.75,fontFamily:"'Inter',sans-serif",style:{pointerEvents:"none"}},line)}),
n.image&&h("image",{href:n.image,x:-30,y:totalH/2-(n.imgH||70)-5,width:60,height:60,preserveAspectRatio:"xMidYMid slice"}),
terms.map(function(term,ti){var tw=term.length*5.5;var ox=(ti-(terms.length-1)/2)*(tw+16);var oy=totalH/2-(n.imgH||0)+2;return h("g",{key:"t"+ti},h("ellipse",{cx:ox,cy:oy,rx:tw/2+6,ry:9,fill:"none",stroke:t.a,strokeWidth:1,opacity:0.5}),h("text",{x:ox,y:oy+1,textAnchor:"middle",dominantBaseline:"central",fontSize:"9",fill:t.a,opacity:0.6,fontWeight:"500",fontFamily:"'Inter',sans-serif"},term))}),
hasCh&&h("g",{transform:"translate("+(n.w/2)+","+(-totalH/2)+")",onClick:function(ev){ev.stopPropagation();coll[1](function(prev){var s2=new Set(prev);if(s2.has(n.id))s2.delete(n.id);else s2.add(n.id);return s2})}},h("circle",{r:10,fill:SURF,stroke:t.a,strokeWidth:0.8}),h("text",{x:0,y:1,textAnchor:"middle",dominantBaseline:"central",fontSize:"9",fill:t.a,fontWeight:"600"},isCol?'+'+(ch[n.id]||[]).length:'−')),
(deg[n.id]||0)>2&&!isHov&&!isSel&&h("g",{transform:"translate("+(-n.w/2)+","+(totalH/2)+")"},h("circle",{r:9,fill:SURF+"CC",stroke:t.a,strokeWidth:0.4}),h("text",{x:0,y:1,textAnchor:"middle",dominantBaseline:"central",fontSize:"8",fill:t.a},deg[n.id])))})),
// Detail card
selN&&(function(){var sc=w2s(selN.x,selN.y);var t=typeColor(P,selN.concept_type);var rc=cRef.current?cRef.current.getBoundingClientRect():{width:800};var cW=280;var cx2=Math.min(Math.max(10,sc.x+80),rc.width-cW-16);var cy2=Math.max(10,sc.y-50);
...
h("div",{style:{display:"flex",alignItems:"center",gap:6,marginBottom:6}},h("div",{style:{width:8,height:8,borderRadius:"50%",background:t.a}}),h("span",{style:{fontSize:12,color:t.a,fontWeight:600,textTransform:"uppercase"}},selN.concept_type),h("span",{style:{fontSize:12,color:DIM,marginLeft:"auto"}},Math.round((selN.confidence||0)*100)+"%"),h("button",{onClick:function(){sel[1](null)},style:{background:"none",border:"none",color:DIM,fontSize:16,cursor:"pointer",padding:"0 4px"}},"×")),
ef[0]==='label'?h("input",{value:ev[0],onChange:function(e){ev[1](e.target.value)},autoFocus:true,onBlur:function(){setData(function(dd){return Object.assign({},dd,{nodes:dd.nodes.map(function(nd){if(nd.id!==sel[0])return nd;var up=Object.assign({},nd,{label:ev[0]});return Object.assign(up,nSize(up))})})});ef[1](null)},onKeyDown:function(e){if(e.key==='Enter')e.target.blur();if(e.key==='Escape')ef[1](null)},style:{width:"100%",fontSize:17,fontWeight:600,background:BG,border:"1px solid "+t.a+"50",borderRadius:8,color:t.a,padding:"4px 8px",marginBottom:6,fontFamily:"inherit"}}):h("h3",{onClick:function(){ef[1]('label');ev[1](selN.label)},style:{fontSize:17,fontWeight:600,marginBottom:6,cursor:"text",color:t.a}},selN.label),
ef[0]==='desc'?h("textarea",{value:ev[0],onChange:function(e){ev[1](e.target.value)},rows:3,autoFocus:true,onBlur:function(){setData(function(dd){return Object.assign({},dd,{nodes:dd.nodes.map(function(nd){if(nd.id!==sel[0])return nd;var up=Object.assign({},nd,{description:ev[0]});return Object.assign(up,nSize(up))})})});ef[1](null)},style:{width:"100%",fontSize:14,background:BG,border:"1px solid "+t.a+"50",borderRadius:8,color:t.s,padding:"6px 8px",marginBottom:8,fontFamily:"inherit",lineHeight:1.5,resize:"vertical"}}):h("p",{onClick:function(){ef[1]('desc');ev[1](selN.description||'')},style:{fontSize:14,color:t.s,lineHeight:1.6,marginBottom:8,cursor:"text"}},selN.description||"Click to add"),
h("div",{style:{display:"flex",gap:4,marginBottom:8}},h("button",{onClick:function(){submitCorrection({map_id:mapId[0],type:"approve",original:{id:sel[0]}}).catch(function(){})},style:B("#51CF66","rgba(81,207,102,0.1)")},"✓ Correct"),h("button",{onClick:function(){setData(function(dd){return{nodes:dd.nodes.filter(function(nd){return nd.id!==sel[0]}),edges:dd.edges.filter(function(ed){return ed.source!==sel[0]&&ed.target!==sel[0]}),drawings:dd.drawings}});sel[1](null)},style:B("#FF6B6B","rgba(255,107,107,0.1)")},"Remove")),
connE.length>0&&h("div",null,h("div",{style:{fontSize:12,color:DIM,fontWeight:600,marginBottom:4}},"Connections ("+connE.length+")"),connE.map(function(e,i){var isSrc=e.source===sel[0],oId=isSrc?e.target:e.source,o=nm[oId];var cat=edgeCat(e.relation_type),es=P.edges[cat]||P.edges.custom;return h("div",{key:i,onClick:function(){selectAndMagnify(oId)},style:{padding:"6px 8px",background:BG,borderRadius:6,marginBottom:3,borderLeft:"3px solid "+es.color,cursor:"pointer"}},h("div",{style:{display:"flex",justifyContent:"space-between",fontSize:12}},h("span",{style:{color:es.color,fontWeight:600,fontSize:10,textTransform:"uppercase"}},(e.relation_type||'').replace(/_/g,' ')),h("span",{style:{color:DIM}},(isSrc?"→ ":"← ")+(o?o.label:"?"))))})))})(),
// Legend
