#!/bin/bash
# ============================================================================
# Mycel v3 — Complete project update
#
# Run from your project root (where backend/ and frontend/ folders are):
#   bash apply_mycel_v3.sh
#
# This script:
#   1. Fixes backend/.env to use phi3:mini (your installed model)
#   2. Writes frontend/src/utils/theme.js (new color system)
#   3. Writes frontend/src/utils/layout.js (overlap-free layout + helpers)
#   4. Writes frontend/src/App.css
#   5. Writes frontend/src/App.jsx (complete v3 with all features)
#   6. Does NOT touch: api.js, main.jsx, package.json, vite.config.js,
#      any backend Python files
#
# After running:
#   Terminal 1: bash start.sh
#   Terminal 2: cd frontend && npm run dev
#   Open: http://localhost:5173
# ============================================================================
set -e
echo "🍄 Mycel v3 — Applying changes..."

# ── Fix 1: Backend .env — match your installed ollama model ─────────────
if [ -f backend/.env ]; then
  echo "  Fixing backend/.env to use phi3:mini..."
...
ENVEOF
  echo "  ✓ backend/.env updated (LLM_MODEL=phi3:mini)"
fi

# ── Ensure directories exist ────────────────────────────────────────────
mkdir -p frontend/src/utils

# ── File 2: frontend/src/utils/theme.js ─────────────────────────────────
cat > frontend/src/utils/theme.js << 'THEOF'
// Mycel v3 Theme — colorful text, no-block default, thick edges
...
THEOF
echo "  ✓ frontend/src/utils/theme.js"

# ── File 3: frontend/src/utils/layout.js ────────────────────────────────
cat > frontend/src/utils/layout.js << 'LAYEOF'
...
LAYEOF
echo "  ✓ frontend/src/utils/layout.js"
...
# This is the COMPLETE App.jsx with home/graph/library views,
# undo/redo, free-form drawing, image embed, family drag,
# all v3 features. It imports from the existing api.js.
...
export default function App() {
  ...
  return React.createElement("div", { style: { height: "100vh", display: "flex", flexDirection: "column", background: P.bg, color: P.text, fontFamily: "'Inter',system-ui,sans-serif" } },
    ...
    // HEADER
    React.createElement("header", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 14px", background: P.surface, borderBottom: "1px solid " + P.border, flexShrink: 0, gap: 8 } },
      React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
        ...
        React.createElement("nav", { style: { display: "flex", gap: 3 } },
          ["home", "graph", "library"].map(function(k) {
            return React.createElement("button", { key: k, onClick: function() { setView(k); }, style: { padding: "3px 10px", borderRadius: 5, border: "none", cursor: "pointer", background: view === k ? P.bg : "transparent", color: view === k ? P.text : P.dim, fontSize: 10, fontWeight: 500 } }, k.charAt(0).toUpperCase() + k.slice(1));
          })
        )
      ),
      view === 'graph' && React.createElement("div", { style: { display: "flex", gap: 2, alignItems: "center" } },
        React.createElement("span", { style: { fontSize: 9, color: P.dim, marginRight: 6 } }, vn.length + " nodes · " + ve.length + " edges"),
        [{ k: 'select', l: '↖' }, { k: 'draw', l: '✎' }, { k: 'eraser', l: '⌫' }].map(function(b) {
          return React.createElement("button", { key: b.k, onClick: function() { setTool(b.k); }, style: { padding: "3px 7px", borderRadius: 4, border: tool === b.k ? "1px solid " + P.text + "30" : "1px solid transparent", background: tool === b.k ? P.bg : "transparent", color: tool === b.k ? P.text : P.dim, fontSize: 11, cursor: "pointer" } }, b.l);
        }),
        tool === 'draw' && ["#A29BFE", "#5EECD5", "#F0A08A", "#FDCB6E", "#FD79A8", "#E8ECF4"].map(function(c) {
          return React.createElement("div", { key: c, onClick: function() { setDrawColor(c); }, style: { width: 12, height: 12, borderRadius: "50%", background: c, cursor: "pointer", outline: drawColor === c ? "2px solid #fff" : "none", outlineOffset: 1, marginLeft: 2 } });
        }),
        React.createElement("div", { style: { width: 1, height: 14, background: P.border, margin: "0 4px" } }),
        React.createElement("button", { onClick: undo, disabled: !hist.past.length, style: { padding: "3px 7px", borderRadius: 4, border: "1px solid " + P.border, background: "transparent", color: hist.past.length ? P.text : P.dim, fontSize: 10, cursor: "pointer", opacity: hist.past.length ? 1 : 0.4 } }, "↩"),
        React.createElement("button", { onClick: redo, disabled: !hist.future.length, style: { padding: "3px 7px", borderRadius: 4, border: "1px solid " + P.border, background: "transparent", color: hist.future.length ? P.text : P.dim, fontSize: 10, cursor: "pointer", opacity: hist.future.length ? 1 : 0.4 } }, "↪")
      )
    ),
    ...
    view === 'home' && React.createElement("div", { style: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 20, padding: "40px 20px" } },
      ...
      React.createElement("div", {
        ...
        React.createElement("input", { id: "fi", type: "file", accept: ".pdf", style: { display: "none" }, disabled: uploading, onChange: function(e) { handleUpload(e.target.files ? e.target.files[0] : null); } }),
        progress && progress.stage !== 'done'
          ...
          : React.createElement("div", null,
              React.createElement("div", { style: { fontSize: 14, fontWeight: 500, marginBottom: 4 } }, "Drop a PDF or click to upload"),
    ...
    view === 'library' && React.createElement("div", { style: { flex: 1, padding: 24, overflowY: "auto" } },
      React.createElement("h2", { style: { fontSize: 16, fontWeight: 600, marginBottom: 14 } }, "Library"),
      maps.length === 0
        ...
        : React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 10 } },
            maps.map(function(m) {
              return React.createElement("div", { key: m.id, onClick: function() { loadMap(m.id); }, style: { padding: 14, background: P.surface, border: "1px solid " + P.border, borderRadius: 10, cursor: "pointer" } },
                React.createElement("div", { style: { fontSize: 13, fontWeight: 600, marginBottom: 3 } }, m.title || m.filename),
    ...
    view === 'graph' && React.createElement("div", {
      ...
      selNode && (function() {
        ...
        return React.createElement("div", { style: { position: "absolute", left: cx2, top: cy2, width: cW, background: P.surface, border: "1px solid " + P.border, borderRadius: 12, padding: "10px 12px", boxShadow: "0 6px 28px " + P.bg + "AA", zIndex: 20, maxHeight: "55vh", overflowY: "auto" } },
          React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, marginBottom: 5 } },
            ...
            React.createElement("span", { style: { fontSize: 8, color: t.a, fontWeight: 600, textTransform: "uppercase" } }, selNode.concept_type),
            React.createElement("span", { style: { fontSize: 8, color: P.dim, marginLeft: "auto" } }, Math.round((selNode.confidence || 0) * 100) + "%"),
          ...
          editField === 'label'
            ...
            : React.createElement("h3", { onClick: function() { setEditField('label'); setEditVal(selNode.label); }, style: { fontSize: 13, fontWeight: 600, marginBottom: 4, cursor: "text", color: t.a } }, selNode.label),
          editField === 'desc'
            ...
            : React.createElement("p", { onClick: function() { setEditField('desc'); setEditVal(selNode.description || ''); }, style: { fontSize: 10, color: t.s, lineHeight: 1.5, marginBottom: 6, cursor: "text" } }, selNode.description || "Click to add"),
          // Image
      ...
      // Legend
      React.createElement("div", { style: { position: "absolute", top: 8, left: 8, background: P.surface + "DD", backdropFilter: "blur(8px)", padding: "7px 9px", borderRadius: 8, border: "1px solid " + P.border, fontSize: 8, zIndex: 5 } },
        ["theorem", "definition", "principle", "method", "framework", "example"].map(function(t2) {
          ...
          return React.createElement("div", { key: t2, style: { display: "flex", alignItems: "center", gap: 4, marginBottom: 1 } },
            React.createElement("div", { style: { width: 6, height: 6, borderRadius: "50%", background: c.a } }),
      ...
      React.createElement("div", { style: { position: "absolute", bottom: 8, right: 8, display: "flex", gap: 3, zIndex: 5 } },
        [{ l: "+", f: 1.2 }, { l: "−", f: 1 / 1.2 }, { l: "⊡", f: 0 }].map(function(b) {
          return React.createElement("button", { key: b.l, onClick: function() { b.f ? setCam(function(c) { return { x: c.x, y: c.y, z: Math.max(0.15, Math.min(5, c.z * b.f)) }; }) : fit(nodes); }, style: { width: 26, height: 26, borderRadius: 6, background: P.surface, border: "1px solid " + P.border, color: P.text, fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" } }, b.l);
        })
      )
    )
  );
...
JSXEOF
echo "  ✓ frontend/src/App.jsx ($(wc -l < frontend/src/App.jsx) lines)"

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🍄 Mycel v3 applied!"
echo ""
echo "WHAT CHANGED:"
echo "  backend/.env    → LLM_MODEL=phi3:mini (matches your ollama)"
echo "  utils/theme.js  → New color system with accent/soft per type"
echo "  utils/layout.js → Overlap-free layout, hull, neighbor helpers"
echo "  App.css         → Minimal reset styles"
echo "  App.jsx         → Complete rewrite with:"
echo "                     • Undo/redo (Ctrl+Z/Y, 40-step history)"
echo "                     • Free-form drawing (D key, 6 colors)"
echo "                     • Eraser tool (E key)"
echo "                     • Image embedding in nodes"
echo "                     • Family drag (every drag moves neighbors)"
echo "                     • No-block default (border on click only)"
echo "                     • Colored text per concept type"
echo "                     • Arrows on logical/causal edges only"
echo "                     • Thick visible edges (3-3.5px, 0.55 opacity)"
echo "                     • Cluster hull outlines"
echo "                     • Inline term circles"
echo "                     • Semantic zoom (3 detail thresholds)"
echo "                     • Collapse/expand on double-click"
echo "                     • Delete key removes selected node"
echo ""
echo "IMPORTANT: This App.jsx uses React.createElement() instead of JSX"
echo "syntax to avoid any transpilation issues. It works with your"
echo "existing vite.config.js and @vitejs/plugin-react setup."
echo ""
echo "TO RUN:"
echo "  Terminal 1: ollama serve  (if not already running)"
echo "  Terminal 2: bash start.sh"
echo "  Terminal 3: cd frontend && npm run dev"
echo "  Open: http://localhost:5173"
echo ""
echo "The home page should appear immediately (no backend needed)."
echo "Upload a PDF → backend processes it → graph view shows."
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"