#!/usr/bin/env bash
# ============================================================================
# Mycel Code — Stage 2 (deterministic connections via name resolution)
#
# Run from repo ROOT:  chmod +x apply_mycel_code_stage2.sh && ./apply_mycel_code_stage2.sh
# Idempotent (rewrites code_parser.py wholesale). Requires Stage 0+1 first
# (needs CodeEntityType / CodeRelationType in models.py).
#
# Adds a second, deterministic resolution pass over the Python AST that emits:
#   CALLS / INSTANTIATES   — the call graph (bare calls + self.method())
#   INHERITS               — class hierarchy (resolved base classes)
#   HAS_TYPE / RETURNS      — from parameter + return annotations (user types only)
#   READS / WRITES          — module/class-level state touched by a function
#
# Every edge is resolved to a KNOWN node only (unresolved names are dropped,
# never guessed) and carries page=line + evidence=source-line — the code analog
# of the concept track's source_page + source_quote, so the v16 splitter UI
# (match badge / evidence) renders code edges with no frontend change.
# ============================================================================
set -e
...
if ! grep -q "class CodeRelationType" "$MODELS" 2>/dev/null; then
  echo "ERROR: CodeRelationType not found in $MODELS."
  echo "       Run the Stage 0+1 script (apply_mycel_code_stage01.sh) first."
  exit 1
fi
echo "  ✓ code enums present in models.py"

mkdir -p "$APP/pipeline"; [ -f "$APP/pipeline/__init__.py" ] || touch "$APP/pipeline/__init__.py"
CP="$APP/pipeline/code_parser.py"
[ -f "$CP" ] && cp "$CP" "$CP.stage1bak" && echo "  ✓ backup: $CP.stage1bak"
...
s = open(path).read()                      # read BEFORE truncating
open(path, "w").write(s.replace("__BASE__", base))
PY
echo "  ✓ wrote $CP (import base: $BASE)"

python3 -m py_compile "$CP" && echo "  ✓ code_parser.py compiles" || {
  echo "  ✗ compile failed — restoring"; [ -f "$CP.stage1bak" ] && mv "$CP.stage1bak" "$CP"; exit 1; }
rm -f "$CP.stage1bak"

# ── smoke test: parse the backend itself, print an edge-type histogram ──
echo "  → smoke test (parsing $APP):"
python3 - "$BASE" "$APP" <<'PY' || echo "     (skipped — run inside your venv to see the histogram)"
import importlib, sys, collections
base, app = sys.argv[1], sys.argv[2]
m = importlib.import_module(base + ".pipeline.code_parser")
g = m.build_code_graph(app)
rt = lambda e: getattr(e.relation_type, "value", e.relation_type)
hist = collections.Counter(rt(e) for e in g.edges)
print("     nodes:", len(g.nodes), "edges:", len(g.edges), "files:", g.metadata.get("files"))
print("     edges:", dict(hist))
new = [k for k in ("CALLS","INSTANTIATES","INHERITS","HAS_TYPE","RETURNS","READS","WRITES") if hist.get(k)]
print("     Stage-2 edge types present:", new or "none (add typed/calling code to see them)")
PY

cat <<'NOTE'

────────────────────────────────────────────────────────────────────────
Mycel Code — Stage 2 applied (deterministic connections).

NEW EDGES (all resolved to known nodes only; every one carries page=line +
evidence=source-line, so the v16 splitter UI shows them like concept relations):
  • CALLS / INSTANTIATES   — call graph (bare calls + self.method())
  • INHERITS               — class hierarchy
  • HAS_TYPE / RETURNS      — from annotations (user-defined types only)
  • READS / WRITES          — module/class-level state a function touches

UNCHANGED: Stage 1 structure (module/class/function/var/param + DEFINES/
CONTAINS/IMPORTS). Node ids are still fully-qualified names → multi-file
resolution and merges are exact.

TEST LOCALLY:
  # pick the "Code" card, upload a .py file (or point the backend at a package)
  # you should now see call/inherit/type edges, not just containment.

NOT YET (later stages):
  • READS/WRITES is conservative (module/class-level names only) — local-var
    dataflow is Stage 2b.
  • Code-specific colours / monospace labels → Stage 3 (theme codeTypes).
  • Other languages via tree-sitter → Stage 5.

DEPLOY:  git add -A && git commit -m "Mycel Code: Stage 2 (name resolution)" && git push
────────────────────────────────────────────────────────────────────────
NOTE
echo "✓ done."